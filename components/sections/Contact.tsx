          ...
          <div className="max-w-4xl w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
              {contactCards.map((card, index) => (
                ...
                >
                  <div className={`absolute inset-0 ${card.glowBg} blur-2xl opacity-0 group-hover:opacity-40 transition-opacity rounded-full`} />
                  <div className={`relative w-16 h-16 rounded-2xl ${card.iconBg} flex items-center justify-center mb-2`}>
                    {card.icon}
                  </div>
                  <span className={`relative font-bold dark:text-white text-gray-900 ${card.hoverText} transition-colors`}>
                    {t(card.titleKey)}
                  </span>
